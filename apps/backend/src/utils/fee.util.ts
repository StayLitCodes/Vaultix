import { FEE_TIERS, DEFAULT_FEE_BPS, BPS_DENOMINATOR } from './fee.constants';

/**
 * Calculate tiered fee basis points based on transaction volume
 * Uses tier-based system where higher volumes get lower fees
 *
 * @param volume - Amount in stroops (10,000,000 stroops = 1 XLM)
...
export function getFeeTier(volume: number): {
  ...
  for (let i = 0; i < FEE_TIERS.length; i++) {
    ...
    if (bps === feeBps) {
      ...
      const lowerXlm = i > 0 ? prevCap / divisor : 0;
      const range = isMaxTier
        ? `${lowerXlm.toLocaleString()}+ XLM`
        : `${lowerXlm.toLocaleString()} - ${(cap / divisor).toLocaleString()} XLM`;
